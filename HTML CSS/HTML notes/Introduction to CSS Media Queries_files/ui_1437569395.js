...
if($iTXT.glob.params.getBool('js.autoplay.channel',false)&&!$iTXT.core.Browser.isSmartphoneOrTablet()&&!$iTXT.glob.params.getBool('js.autoplay.mobileonly',false)){var deciding=false,quiet_time=false;var quiet_period=$iTXT.glob.params.getInt('js.autoplay.quiet',2000);var seen_ionos={},adtypes=[],page_lim=$iTXT.glob.params.getInt("js.autoplay.pagelimit",5),all_views=0,first_fire=true;if($iTXT.glob.params.get("js.autoplay.geoblock")){var blockedGeos=$iTXT.glob.params.get("js.autoplay.geoblock").split(",");var endUserCountry=$iTXT.glob.params.get("cc");var len=blockedGeos.length;var i;for(i=0;i<len;i++){if(blockedGeos[i]===endUserCountry){return false;}}}
if($iTXT.glob.params.get('js.autoplay.adtypes',false)){adtypes=$iTXT.glob.params.get('js.autoplay.adtypes','').split(",");adtypes=VM._.map(adtypes,function(el){return+el;});}
...
setTimeout(function(){el.className+=' animate';},20);},flipMobt:function(left,l_offset,t_offset){if(this.hasFlipped&&this.ad.params.get("js.autoplay",false)){this.ad.params.set("nott",0,9999);return;}
